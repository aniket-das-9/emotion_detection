...
def load_data(data_url: str) -> pd.DataFrame:
    try:
        ...
        logger.error(f"Failed to parse the CSV file from {data_url}.", exc_info=True)
        # print(f"Error: Failed to parse the CSV file from {data_url}.")
        # print(e)
        raise
    except Exception as e:
        logger.critical("An unexpected error occurred while loading the data.", exc_info=True)
...
def preprocess_data(df: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        logger.error(f"Missing column {e} in the dataframe.", exc_info=True)
        # print(f"Error: Missing column {e} in the dataframe.")
        raise
    except Exception as e:
        logger.critical("An unexpected error occurred during preprocessing.", exc_info=True)
